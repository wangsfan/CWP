...
def train_one_epoch(model: torch.nn.Module, model_old: torch.nn.Module,
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        with torch.no_grad():
            if model_old is not None:
                output = model_old(input, task_id=task_id, train=False)
        ...
        l_match = (1.0 - output['similarity_task']).sum()
        loss = loss + 0.1 * l_match.sum()
# 
...
@torch.no_grad()
def evaluate(model: torch.nn.Module, model_old: torch.nn.Module, data_loader, 
...
@torch.no_grad()
def evaluate_till_now(model: torch.nn.Module, model_old: torch.nn.Module, data_loader, 
    ...
    for i in range(task_id+1):
        ...
        test_stats, temp_total, temp_correct = evaluate(model=model, model_old=model_old, data_loader=data_loader[i]['val'], 
    ...
    # print(a)
    # np.savez('matrices.npz', matrix_1=a)
...
def train_and_evaluate(model: torch.nn.Module, model_old: torch.nn.Module,
    ...
    for task_id in range(args.num_tasks):
        ...
        for epoch in range(args.epochs):     
            ...
            train_stats = train_one_epoch(model=model, model_old=model_old, criterion=criterion, 
        ...
        test_stats = evaluate_till_now(model=model, model_old=model_old, data_loader=data_loader, device=device, 
...
@torch.no_grad()
def save_feature(model: torch.nn.Module, model_old: torch.nn.Module, data_loader, 
            device, task_id=-1, class_mask=None, args=None,):
...
@torch.no_grad()
def save_feature_till_now(model: torch.nn.Module, model_old: torch.nn.Module, data_loader, 
                    device, task_id=-1, class_mask=None, acc_matrix=None, args=None,):
    stat_matrix = np.zeros((3, args.num_tasks)) # 3 for Acc@1, Acc@5, Loss
