...
def build_continual_dataloader(args):
    ...
    if args.dataset.startswith('Split-'):
        ...
        if args.dataset.replace('Split-','') == 'CIFAR100':
            ...
            # class_list = [dataset_train.classes[i:i + classes_per_task] for i in range(0, len(dataset_train.classes), classes_per_task)]
            class_list = dataset_train.classes
        if args.dataset.replace('Split-','') == 'Imagenet-R':
            classes_list = []
            with open('./data/imagenet-r/README.txt', 'r') as file:
                lines = file.readlines()
                for line in lines:
                    a = line.strip().split(' ')
                    classes_list.append(a[1])

            args.nb_classes = len(classes_list)
            classes_per_task = args.nb_classes // args.num_tasks
            # class_list = [classes_list[i:i + classes_per_task] for i in range(0, len(classes_list), classes_per_task)]
            class_list = classes_list

        if args.dataset.replace('Split-','') == 'CUB200':
            print('dfh')
            classes_list = []
        ...
        if args.dataset.replace('Split-','') == 'EuroSAT':
            print('dfh')
            classes_list = []
            with open('./data/class_name.json', 'r') as file:
                class_name = json.load(file)
            for i in range(len(class_name)):
                classes_list.append(class_name[str(i)])
    ...
    return dataloader, class_mask, class_list
...
def get_dataset(dataset, transform_train, transform_val, args,):
    if dataset == 'CIFAR100':
        ...
        dataset_train = EuroSAT(args.data_path, train=True, download=True, transform=transform_train).data
        dataset_val = EuroSAT(args.data_path, train=False, download=True, transform=transform_val).data
        dataset_feat_train = EuroSAT(args.data_path, train=True, download=True, transform=transform_val).data
    else:
