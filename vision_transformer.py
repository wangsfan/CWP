...
class PromptLearner(nn.Module):
    def __init__(self, args = None,clip_model =None,class_list=None):
        ...
        self.class_list = [self.classes_list[i:i + 2] for i in range(0, 10, 2)]  #imagenet-r
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            prompts_per_task=5, args=None,class_list=None):
        """
        Args:
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            self.e_prompt = EPrompt(length=prompt_length, embed_dim=embed_dim, num_tasks=self.num_tasks, kernel_size=kernel_size, embedding_key=embedding_key, prompt_init=prompt_init,
                    ...
                    num_heads=num_heads, same_key_value=same_key_value, prompts_per_task=prompts_per_task,class_list=class_list,args=args)
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            ...
            self.key_task = PromptLearner(args=args, clip_model=clip_model,class_list=class_list)
            self.tokenized_prompts_key_task_class = self.key_task.tokenized_prompts_class
            self.tokenized_prompts_key_task = self.key_task.tokenized_prompts_task
            self.class_name = torch.nn.Parameter(torch.zeros(10,768), requires_grad=False)
            self.task_name = torch.nn.Parameter(torch.zeros(5,768), requires_grad=False)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        if self.training:
            if self.use_e_prompt:
                ...
                tokenized_prompts_task = self.tokenized_prompts_key_task
                text_features_task = self.text_encoder(prompts_task, tokenized_prompts_task)

                self.class_name.data.copy_(text_features_class)
                self.task_name.data.copy_(text_features_task)
        else:
            if self.use_e_prompt:
                text_features_class = self.class_name
                text_features_task = self.task_name
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        ...
                        res = self.e_prompt(x, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=x[:, 0],
                                            text_features_class=text_features_class,text_features_task=text_features_task,cls_features_task=cls_features)
